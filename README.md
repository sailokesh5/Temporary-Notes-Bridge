...
- **One-tap Copy / Save / Delete** per image on the receiver view, designed for touch devices
...
|---|---|---|---|
| `127.0.0.1:5000` | Laptop (loopback) | `sender.html` | Paste box, full-size preview of what was sent, expiry countdown, Delete |
| `<lan-ip>:5000` | Any other device (e.g. iPad) | `index.html` | Same, plus Copy and Save (for pulling the image into notes) |

The laptop doesn't need Copy/Save since the original screenshot already exists on its own disk/clipboard — it only needs to confirm the right image was sent and be able to delete a mistake.
...
flowchart LR
    ...
    B -. "serves sender.html<br/>(no Copy/Save)" .-> A
    E["iPad Browser<br/>(LAN IP)"] -- "polls /api/images every 4s" --> B
    B -. "serves index.html<br/>(Copy/Save/Delete)" .-> E
```
...
├── templates/
│   ├── index.html           # Receiver UI (Copy/Save/Delete) — served to non-loopback devices
